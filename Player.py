...
def generate_population( model, minModels = False ):
    # [OBS, MOVES]
    ...
    while _ < initial_games:
        _ = _ + 1
        print('Simulation ', str( _ ).zfill( 4 ), "Passing: ", str( len( training_data ) ), " out of ", str( initial_games ), '\r', end='')
    ...
    print()
    print( len( accepted_scores ), accepted_scores )
    median_accepted_scores = 0
    if len( accepted_scores ) > 0:
      score_requirement = mean(accepted_scores)
      median_accepted_scores = median( accepted_scores )
    print('Average accepted score:', score_requirement)
    print('Score Requirement:', score_requirement)
    print('Median score for accepted scores:', median_accepted_scores)
...
def evaluate(model, addedText = '' ):
    ...
    for each_game in range( 2 ):
        ...
        prev_obs = []
        env.reset()
        env.setAddedText( addedText + ' ' + str( each_game + 1 ).zfill( 2 ) )
        done = False
        while not done:
            env.render()
...
if __name__ == "__main__":
    ...
    # initial_population
    training_data = generate_population( None, True )
    ...
    generation = 1
    # evaluating
    evaluate( model, 'Generation: ' + str( generation ) )
    ...
    while True:
        ...
        newPop = generate_population(model)

        print( 'training_data', len( training_data ) )
        print( 'generate_population', len( newPop ) )
        training_data = np.append(training_data, newPop, axis=0)
